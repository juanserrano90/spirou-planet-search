...
def load_lsd_spectral_lines(p, loc, verbose=False):
    ...
    dmask = np.where(np.logical_and(depth > p['IC_POLAR_LSD_MIN_LINEDEPTH'],depth < p['IC_POLAR_LSD_MAX_LINEDEPTH']))
    # apply mask to the data
    ...
    loc['LSD_NORM_WLC'] = loc["MEAN_WAVE_OF_LINES"]
    if (type(p['IC_POLAR_LSD_NORM_DEPTH']) == float or type(p['IC_POLAR_LSD_NORM_DEPTH']) == int) and p['IC_POLAR_LSD_NORM_DEPTH'] != 0 :
        loc['LSD_NORM_DEPTH'] = p['IC_POLAR_LSD_NORM_DEPTH']
    else :
        loc['LSD_NORM_DEPTH'] = np.nanmean(depth)

    loc['LSD_NORM_LANDE'] = loc["MEAN_LANDE_OF_LINES"]
    loc['LSD_NORM_WEIGHT'] = loc['LSD_NORM_WLC'] * loc['LSD_NORM_DEPTH'] * loc['LSD_NORM_LANDE']
    if verbose :
        print("LSD normalization values: wl={0:.3f} nm depth={1:.3f} lande={2:.3f} weight={3:.3f}".format(loc['LSD_NORM_WLC'], loc['LSD_NORM_DEPTH'], loc['LSD_NORM_LANDE'], loc['LSD_NORM_WEIGHT']))

    weight = weight / loc['LSD_NORM_WEIGHT']
    depth = depth / loc['LSD_NORM_DEPTH']
...
def prepare_polarimetry_data(p, loc):
    ...
    for order_num in range(ydim):
        # mask NaN values
        nanmask = np.isfinite(loc['STOKESI'][order_num])
        nanmask &= np.isfinite(loc['POL'][order_num])
        nanmask &= np.isfinite(loc['POLERR'][order_num])
        nanmask &= np.isfinite(loc['NULL2'][order_num])
        nanmask &= np.isfinite(loc['STOKESIERR'][order_num])
        ...
        if len(wl_tmp[mask]):
            ...
            if p['IC_POLAR_LSD_NORMALIZE']:
                ...
                #plt.plot(wl, cont, "-")
                #plt.errorbar(wl, flux, yerr=fluxerr,fmt=".",alpha=0.4,color="k")
                #plt.show()
                # normalize flux
                flux = flux / cont
            ...
            medsig_flux = np.nanmedian(np.abs(flux - med_flux)) / 0.67449
            clean_outliers = flux < (med_flux + 3.0 * medsig_flux)
            clean_outliers &= flux > (med_flux - 3.0 * medsig_flux)
            med_flux = np.nanmedian(flux[clean_outliers])
            medsig_flux = np.nanmedian(np.abs(flux[clean_outliers] - med_flux)) / 0.67449
            clean_outliers = flux < (med_flux + 3.0 * medsig_flux)
            
            if len(flux[clean_outliers] > 500) :
                # append data to output vector
                loc['LSD_WAVE'] = np.append(loc['LSD_WAVE'], wl[clean_outliers])
                loc['LSD_FLUX'] = np.append(loc['LSD_FLUX'], flux[clean_outliers])
                loc['LSD_FLUXERR'] = np.append(loc['LSD_FLUXERR'], fluxerr[clean_outliers])
                loc['LSD_POL'] = np.append(loc['LSD_POL'], pol[clean_outliers])
                loc['LSD_POLERR'] = np.append(loc['LSD_POLERR'], polerr[clean_outliers])
                loc['LSD_NULL'] = np.append(loc['LSD_NULL'], null[clean_outliers])
                """
               if len(flux) > 500:
    ...
    #plt.errorbar(loc['LSD_WAVE'], loc['LSD_FLUX'], yerr=loc['LSD_FLUXERR'],fmt=".",alpha=0.4, color="k")
    #plt.plot(loc['LSD_WAVE'], loc['LSD_FLUX'], "-", lw=0.3)
    #plt.errorbar(loc['LSD_WAVE'], loc['LSD_NULL'], yerr=loc['LSD_POLERR'],fmt=".",alpha=0.4, color="k")
    #plt.plot(loc['LSD_WAVE'], loc['LSD_NULL'], "-", lw=0.3)
    #plt.show()
...
def lsd_analysis(p, loc):
    ...
    if p['IC_POLAR_LSD_NORM_LSD_STOKES_I'] :
        nflux = 1.0 - loc['LSD_STOKESI']
        edgesize = p['IC_POLAR_LSD_NEDGEPOINTS_FOR_STOKES_I_CONTINUUM']
        cont_sample  = np.append(nflux[:edgesize],nflux[-edgesize:])
        cont_vels = np.append(loc['LSD_VELOCITIES'][:edgesize],loc['LSD_VELOCITIES'][-edgesize:])

        if p['IC_POLAR_LSD_FITLINE_TO_STOKES_I_CONTINUUM'] :
            contfit = np.polyfit(cont_vels, cont_sample, 1)
            pfit = np.poly1d(contfit)
            stokes_I_continuum = pfit(loc['LSD_VELOCITIES'])
        else :
            stokes_I_continuum = np.nanmedian(cont_sample)

        if p['IC_POLAR_LSD_PLOT_STOKES_I_CONTINUUM'] :
            plt.plot(loc['LSD_VELOCITIES'],nflux,'-',label="Unnormalized profile")
            plt.plot(cont_vels,cont_sample,'o',label="Selected continnum points")
            plt.plot(loc['LSD_VELOCITIES'],np.full_like(loc['LSD_VELOCITIES'],stokes_I_continuum),'--',label="Measured continuum")
            plt.plot(loc['LSD_VELOCITIES'],np.full_like(loc['LSD_VELOCITIES'],np.nanmedian(cont_sample)),'--',label="Median of continuum points")
            plt.ylabel("Stokes I",fontsize=18)
            plt.xlabel("Velocity [km/s]",fontsize=18)
            plt.legend()
            plt.show()
        
        loc['LSD_STOKESI'] = (1.0 - loc['LSD_STOKESI']) / stokes_I_continuum
        loc['LSD_STOKESI_ERR'] /= stokes_I_continuum
    else :
        loc['LSD_STOKESI'] = (1.0 - loc['LSD_STOKESI']) / np.nanmedian(1.0 - loc['LSD_STOKESI'])
...
def line_pattern_matrix(wl, wlc, depth, weight, vels):
    ...
    dvel = np.nanmedian(np.abs(vels[1:] - vels[:-1]))
    ...
    for l in range(len(wlc)):
        ...
        #print("processing line {}/{} wl={:.3f} nm".format(l+1,len(wlc),wlc[l]))
        for i in range(len(line)) :
            j = np.searchsorted(vels, vl[i], side='right')
            
            mmp[line[i],j-1] += weight[l] * (vels[j] - vl[i]) / dvel
            mmp[line[i],j] += weight[l] * (vl[i] - vels[j-1]) / dvel

            mm[line[i],j-1] += depth[l] * (vels[j] - vl[i]) / dvel
            mm[line[i],j] += depth[l] * (vl[i] - vels[j-1]) / dvel
...
def polar_lsd_plot(p, loc):
    ...
    # ---------------------------------------------------------------------
    # set up fig
    fig, frames = setup_figure(p, ncols=1, nrows=3, sharex=True)
...
def save_lsd_fits(filename, loc, p) :
    ...
    header.set('WAVENORM', loc["LSD_NORM_WLC"], 'Normalization wavelength used in LSD')
    header.set('DEPTNORM', loc["LSD_NORM_DEPTH"], 'Normalization line depth used in LSD')
    header.set('LANDNORM', loc["LSD_NORM_LANDE"], 'Normalization lande used in LSD')
    header.set('WEIGNORM', loc["LSD_NORM_WEIGHT"], 'Normalization weight = wl * d * g used in LSD')
...
def setup_figure(p, figsize=(10, 8), ncols=1, nrows=1, attempt=0, sharex=True):
    """
