...
def plot_lsd_profiles(vels, zz, zz_err, zgauss, z_p, z_p_err, z_p_model, z_np, z_np_err, vel_lim=None, stokes="V"):
    ...
    mvels = np.arange(np.min(vels), np.max(vels), 0.01)
    mzgauss = interp1d(vels, zgauss, kind="cubic")
    frame.plot(mvels, mzgauss(mvels), '-', color='green')
    if vel_lim != None :
    ...
    frame.plot(vels, z_p, '-', color='blue')
    #mz_p_model = interp1d(vels, z_p_model, kind="cubic")
    #frame.plot(mvels, mz_p_model(mvels), '-', linewidth=1.0, color='#1f77b4')
    if vel_lim != None :
        frame.set_xlim(vel_lim[0],vel_lim[1])
...
def longitudinal_b_field_montecarlo(vels, pol, pol_err, flux, flux_err, l0, geff, nsamples = 1000, plot=False, verbose=False) :

    const = - 2.14e11 / (l0 * geff * (constants.c / 1000.))
    
    pol_samples, flux_samples = [], []
    for i in range(len(pol)) :
        pol_samples.append(np.random.normal(pol[i], pol_err[i], nsamples))
    for i in range(len(flux)) :
        flux_samples.append(np.random.normal(flux[i], flux_err[i], nsamples))
    pol_samples = np.array(pol_samples, dtype=float)
    flux_samples = np.array(flux_samples, dtype=float)

    blong_samples = np.array([])
    for s in range(nsamples) :
        int_pol = simps(vels * pol_samples[:,s], x=vels)
        int_flux = simps((1.0 - flux_samples[:,s]), x=vels)
        blong_samples = np.append(blong_samples,const * (int_pol / int_flux))

    blong_percentiles = np.percentile(blong_samples, [16, 50, 84], axis=0)
    blong = blong_percentiles[1]
    blong_max_err = blong_percentiles[2]-blong_percentiles[1]
    blong_min_err = blong_percentiles[1]-blong_percentiles[0]
    blongerr = (blong_max_err + blong_min_err) / 2

    if verbose :
        print("Bl = {0:.2f} + {1:.2f} - {2:.2f} G".format(blong, blong_max_err, blong_min_err))
              
    if plot :
        count, bins, ignored = plt.hist(blong_samples, 30, density=True)
        plt.plot(bins, 1/(blongerr * np.sqrt(2 * np.pi)) * np.exp( - (bins - blong)**2 / (2 * blongerr**2) ), linewidth=2, color='r')
        plt.show()

    return blong, blongerr
...
def fit_zeeman_split(vels, pol, pol_err, guess=None, func_type="voigt", plot=False) :
    ...
    if func_type == "gaussian" :
        ...
        try :
            ...
            print("WARNING: could not fit Gaussian profile, setting null model")
            pfit = pcov =  [0., 0., 0., 0., 0.]
        ...
        try :
            ...
            print("WARNING: could not fit Voigt profile, setting null model")
            pfit = pcov = [0., 0., 0., 0., 0., 0.]
...
def save_lsdstack_to_fits(filename, vels, flux, fluxerr, fluxmodel, pol, polerr, polmodel,  null, nullerr, base_header, blong_vel_min=-50, blong_vel_max=50) :
    ...
    header.set('WAVENORM', base_header["WAVENORM"], 'Normalization wavelength used in LSD')
    header.set('DEPTNORM', base_header["DEPTNORM"], 'Normalization line depth used in LSD')
    header.set('LANDNORM', base_header["LANDNORM"], 'Normalization lande used in LSD')
    header.set('WEIGNORM', base_header["WEIGNORM"], 'Normalization weight = wl * d * g used in LSD')
